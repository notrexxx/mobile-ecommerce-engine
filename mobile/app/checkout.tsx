...
export default function CheckoutScreen() {
  ...
  const handlePayment = async () => {
    if (!address || !city || !zipCode) {
      Toast.show({ type: 'error', text1: 'Missing Information', text2: 'Please fill out all shipping details.' });
      return;
    }

    if (cart.length === 0) {
      Toast.show({ type: 'info', text1: 'Empty Cart', text2: 'Please add items before checking out.' });
      return;
    }
    ...
    try {
      ...
      // THE FIX: Changed 'total' to 'totalAmount' to perfectly match your Supabase schema
      ...
      if (orderError) throw new Error(orderError.message || orderError.details || 'Database rejected the order payload.');
